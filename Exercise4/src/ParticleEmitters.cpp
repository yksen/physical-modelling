...
StaticBoxEmitter::StaticBoxEmitter(float emit_rate, ofVec3f origin, ofVec3f range) : ParticleEmitter(emit_rate, origin)
{
...
StaticSphereEmitter::StaticSphereEmitter(float emit_rate, ofVec3f origin, float radius) : ParticleEmitter(emit_rate, origin)
{
    addGenerator(common::short_time_generator);
    addGenerator(common::random_color_generator);
    addGenerator(std::make_shared<SpherePositionGenerator>(&this->origin, radius));
}

ExplosionEmitter::ExplosionEmitter(float emit_rate, ofVec3f origin, float power, ofColor min_color, ofColor max_color) : ParticleEmitter(emit_rate, origin)
{
    addGenerator(std::make_shared<TimeGenerator>(0.5f, 1.f));
    addGenerator(std::make_shared<RandomColorGenerator>(min_color, max_color));
    addGenerator(std::make_shared<SpherePositionGenerator>(&this->origin, 1));
    addGenerator(std::make_shared<VelocityGenerator>(ofVec3f(-power), ofVec3f(power)));
}

SnowfallEmitter::SnowfallEmitter(float emit_rate, ofVec3f origin, float radius) : ParticleEmitter(emit_rate, origin)
{
